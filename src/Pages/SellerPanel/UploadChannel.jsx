...
export default function UploadChannel() {
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-[#0d0b1a] flex items-center justify-center px-5 pt-24">
        <motion.div
          className="bg-white/[0.04] backdrop-blur-2xl border border-emerald-500/20 rounded-3xl p-10 text-center max-w-md w-full"
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            ...
            {!isEditMode && (
              <button
                ...
                className="px-5 py-2.5 bg-white/[0.06] border border-white/10 text-white/70 font-medium rounded-xl text-sm hover:bg-white/10 transition-colors"
              >
                List Another
              </button>
            )}
  ...
  return (
    <div className="min-h-screen bg-[#0d0b1a] flex items-start justify-center px-5 pt-24 pb-12">
      <div className="w-full max-w-3xl flex flex-col gap-7">
              ...
              <React.Fragment key={s.id}>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className={`text-xs font-semibold hidden sm:block transition-colors duration-300 ${
                    isDone ? 'text-emerald-400' : isActive ? 'text-purple-400' : 'text-white/30'
                  }`}>{s.label}</span>
                </div>
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-px mx-2 transition-all duration-500 ${isDone ? 'bg-emerald-500/40' : 'bg-white/10'}`} />
                )}
              </React.Fragment>
            );
          })}
        ...
        <motion.div
          key={step}
          className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-2xl p-8"
        ...
        >
            ...
            <div>
              ...
              <div className="mb-6 rounded-2xl border border-purple-500/25 bg-gradient-to-br from-purple-500/[0.08] to-fuchsia-500/[0.04] p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-bold text-white">Auto-fill from YouTube</span>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white/[0.06] border border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-white placeholder-white/25 outline-none focus:border-purple-500 focus:bg-purple-500/[0.08] transition-all"
                  />
                  <button
                ...
                {fetchedChannel && (
                  <motion.div
                    className="mt-6 relative bg-[#0f0f0f] border border-white/10 rounded-2xl p-5 sm:p-6 overflow-hidden"
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 relative z-10">
                      {/* Avatar */}
                      <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full overflow-hidden shrink-0 border border-white/10 shadow-xl bg-black">
                      ...
                      <div className="flex-1 flex flex-col sm:flex-row gap-4 justify-between min-w-0 text-center sm:text-left w-full">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 text-[0.85rem] text-white/60 mb-2.5">
                            <span className="font-medium text-white">{fetchedChannel.customUrl || '@channel'}</span>
                          ...
                          <p className="text-[0.8rem] text-white/50 line-clamp-2 mb-3 leading-relaxed max-w-xl">
                    ...
                    {(form.monetized || form.estimatedEarnings) && (
                      <div className="mt-5 pt-4 border-t border-white/10 flex flex-wrap items-center justify-center sm:justify-start gap-2 relative z-10">
                        {form.monetized && (
                    ...
                    <button onClick={clearFetch} className="absolute top-3 right-3 text-white/20 hover:text-white transition-colors z-20 bg-black/50 p-1.5 rounded-full" title="Clear">
                      <X size={14} />
                    </button>
                  </motion.div>
                )}
          ...
          {step === 1 && (
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label={<span>Asking Price <span className="normal-case text-white/50 ml-1 font-normal tracking-normal">(Write your Selling price in Dollar)</span></span>} icon={<DollarSign size={13} />} error={fieldErrors.price}>
              ...
              {/* Banner Upload */}
                  <div className="mb-6">
                    <p className="text-xs font-semibold text-white/50 mb-2">
                      Banner / Thumbnail <span className="font-normal text-white/30">(1 image required)</span>
                    </p>
                    {bannerPreview ? (
                      ...
                      <label className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl p-10 cursor-pointer transition-all
                        ${fieldErrors.banner ? 'border-red-500/40' : 'border-white/[0.12] hover:border-purple-500 hover:bg-purple-500/[0.06]'}
                  ...
                  <div>
                    <p className="text-xs font-semibold text-white/50 mb-2">
                      Channel Screenshots <span className="font-normal text-white/30">(2–4 images required)</span>
                    </p>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
                      {imagePreviews.map((src, i) => (
                        <div key={i} className="relative rounded-xl overflow-hidden aspect-video bg-white/5">
                          ...
                          <button
                            ...
                            className="absolute top-1.5 right-1.5 w-6 h-6 bg-black/60 hover:bg-red-500 rounded-full flex items-center justify-center text-white transition-colors"
                          ><X size={12} /></button>
                        </div>
                      ))}
                      {(existingImages.length + images.length) < 4 && (
                        <label className={`flex flex-col items-center justify-center gap-1.5 aspect-video border-2 border-dashed rounded-xl cursor-pointer transition-all text-xs font-medium
                          ${fieldErrors.images ? 'border-red-500/40 text-red-400' : 'border-white/[0.12] hover:border-purple-500 hover:bg-purple-500/[0.06] text-white/30 hover:text-purple-400'}`}>
                          <Upload size={18} />
                          <span>Add Image</span>
          ...
          <div className="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-white/[0.06]">
            {step > 0 && (
              <button
                onClick={prevStep}
                className="flex items-center gap-2 px-5 py-2.5 bg-white/[0.06] border border-white/10 text-white/70 font-medium rounded-xl text-sm hover:bg-white/10 transition-colors"
              >
...
function Field({ label, icon, error, children }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="flex items-center gap-1.5 text-[0.72rem] font-semibold text-white/50 uppercase tracking-wide">
      ...
      <div className={`[&_input]:w-full [&_select]:w-full [&_textarea]:w-full
        ...
        ${error
          ...
          : '[&_input]:border-white/10 [&_select]:border-white/10 [&_textarea]:border-white/10'
        }
    ...
    <label className="flex items-center gap-3 cursor-pointer select-none group">
      <div
        className={`relative w-11 h-6 rounded-full border transition-all duration-300 shrink-0 ${
          checked ? 'bg-purple-600 border-purple-500' : 'bg-white/10 border-white/15'
        }`}
