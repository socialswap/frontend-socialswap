...
const Carousel = ({ children }) => {
  ...
  // --- replace the existing autoScroll function and useEffect with this ---

const autoScroll = () => {
  const container = containerRef.current;
  if (!container || isPaused) return;

  const tolerance = 1; // account for precision
  // if there is nothing to scroll, bail out
  if (container.scrollWidth <= container.offsetWidth + tolerance) return;

  const isAtEnd =
    container.scrollLeft + container.offsetWidth >= container.scrollWidth - tolerance;

  // optional: only log in dev (vite)
  if (import.meta?.env?.DEV) console.log('isAtEnd:', isAtEnd);

  if (isAtEnd) {
    container.scrollTo({ left: 0, behavior: 'smooth' });
  } else {
    // scroll by a sensible amount (adjust as needed)
    const step = Math.min(300, container.scrollWidth - container.offsetWidth);
    container.scrollBy({ left: step, behavior: 'smooth' });
  }
};

useEffect(() => {
  // start the interval only if the container is rendered and scrollable
  const startAutoIfScrollable = () => {
    const c = containerRef.current;
    if (!c || c.scrollWidth <= c.offsetWidth + 1) return;
  ...
  const tId = setTimeout(startAutoIfScrollable, 100);

  // if window is resized after mount, (re)start or stop auto scroll appropriately
  const onResize = () => {
    if (autoScrollRef.current) {
      clearInterval(autoScrollRef.current);
      autoScrollRef.current = null;
    }
    startAutoIfScrollable();
  };
...
}, [isPaused]); // same dependencies as before
